...
func InsertOrder(o *models.Order) error {
	...
	_, err := db.DB().Exec(s, o.ID, o.UserID, o.OrderDate, o.Status, o.PaymentURL)
	return err
}
...
func InsertOrderProducts(orderID string, items []struct{ ProductID int64; Quantity int }) error {
	...
	for _, it := range items {
		if _, err := db.DB().Exec(s, orderID, it.ProductID, it.Quantity); err != nil { return err }
	}
	return nil
}

func ListOrders() ([]models.Order, error) {
	rows, err := db.DB().Query(`SELECT id, user_id, order_date, status, payment_url FROM orders`)
...
func GetOrderByID(id string) (*models.Order, error) {
	row := db.DB().QueryRow(`SELECT id, user_id, order_date, status, payment_url FROM orders WHERE id = ?`, id)
	o := models.Order{}
...
func GetOrdersByUserID(userID int64) ([]models.Order, error) {
	rows, err := db.DB().Query(`SELECT id, user_id, order_date, status, payment_url FROM orders WHERE user_id = ?`, userID)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []models.Order
	for rows.Next() {
...
func GetOrdersByProductID(productID int64) ([]models.Order, error) {
	...
	rows, err := db.DB().Query(s, productID)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []models.Order
	for rows.Next() {
...
func UpdateOrderStatus(orderID string, status models.OrderStatus) (bool, error) {
	res, err := db.DB().Exec(`UPDATE orders SET status = ? WHERE id = ?`, status, orderID)
