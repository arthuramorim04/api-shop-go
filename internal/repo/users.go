...
func CreateUser(u *models.User) (int64, error) {
	...
	res, err := database.DB().Exec(s, u.Username, u.Email, u.Password, u.FirstName, u.LastName, u.Address, u.PhoneNumber, u.Role)
	if err != nil { return 0, err }
	return res.LastInsertId()
}

func GetUserByID(id int64) (*models.User, error) {
	row := database.DB().QueryRow(`SELECT id, username, email, password, firstName, lastName, address, phoneNumber, role FROM users WHERE id = ?`, id)
	u := models.User{}
...
func GetUserByEmail(email string) (*models.User, error) {
	row := database.DB().QueryRow(`SELECT id, username, email, password, firstName, lastName, address, phoneNumber, role FROM users WHERE email = ?`, email)
	u := models.User{}
	if err := row.Scan(&u.ID, &u.Username, &u.Email, &u.Password, &u.FirstName, &u.LastName, &u.Address, &u.PhoneNumber, &u.Role); err != nil {
		if errors.Is(err, sql.ErrNoRows) { return nil, nil }
		return nil, err
...
func ListUsers() ([]models.User, error) {
	rows, err := database.DB().Query(`SELECT id, username, email, password, firstName, lastName, address, phoneNumber, role FROM users`)
...
func UpdateUser(id int64, u *models.User) (bool, error) {
	...
	res, err := database.DB().Exec(s, u.Username, u.Email, u.FirstName, u.LastName, u.Address, u.PhoneNumber, u.Role, id)
