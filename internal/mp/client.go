...
// Client defines the Mercado Pago operations used by the app.
// This enables generating mocks with mockery for unit tests.
type Client interface {
    GetPayment(accessToken, paymentID string) (string, error)
    CreatePreference(accessToken string, pref PreferenceRequest) (string, error)
}

// httpClient is the production implementation of Client using net/http.
...
func (httpClient) GetPayment(accessToken, paymentID string) (string, error) {
...
func (httpClient) CreatePreference(accessToken string, pref PreferenceRequest) (string, error) {
...
// ClientImpl is the package-level client used in production.
// Tests can replace this with a mock.
var ClientImpl Client = httpClient{}
